...
UsbDevice* UsbDevice::createFromConnectedDevice(u_int16_t vendorID,
		u_int16_t productID) {
	libusb_init(NULL);
	libusb_device_handle *targetDeviceHandle = libusb_open_device_with_vid_pid(NULL, vendorID, productID);
...
bool UsbDevice::sendControlMsg(int rquestType, int request, int wValue,
	...
	int result = libusb_control_transfer(deviceHandle, rquestType, request, wValue,
